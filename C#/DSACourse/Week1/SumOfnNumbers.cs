...
namespace Week1 {
    public class SumOfnNumbers {
        ...
        public int SumUsingSingleLoop (int n) {
            ...
            //c1 = 6, c2 = 2 (c1 is i=0, i<=n, i+1, i={i+1}, sum+i, sum={sum+i} and c2 is sum=0, return sum)
            int sum = 0;
            for (int i = 0; i <= n; i++) {
                sum = sum + i;
            }
        ...
        public int SumUsingMultipleLoop (int n) {
            ...
            //c1 = 6, c2 = 4, c3 = 2 (c1 is j=1, j<=i, j+1, j={j+1}, sum+1, sum={sum+1} and c2 is i=1, i<=n, i+1, i={i+1} and c3 is sum=0, return sum)
            //for n=3, outer loop will execute 3 times, inner loop will execute 6 times, so a total of 9 executions when n=3 which is n*n.
            int sum = 0;
            for (int i = 1; i <= n; i++) {
                for (int j = 1; j <= i; j++) {
                    sum++;
